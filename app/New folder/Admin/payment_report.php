	...
	<div class="main-content-inner">
		<div class="breadcrumbs ace-save-state" id="breadcrumbs">
			<ul class="breadcrumb">
				...
				<li>
					<a href="<?php echo base_url(); ?>Admin/payment">Order Report</a>
				</li>							
		...
		<div class="page-content">
			...
			<div class="row">
			<form action="<?php echo base_url();?>Admin/payment_report" method="post">			
				...
				<div class="col-sm-3">
					<div class="form-group">
						<label>Client </label>
						<select class="form-control font-mark-form" id="sel1" name="buyer">
						<option value="">Select</option>
						<?php if(count($buyer_list) > 0){ 
						foreach($buyer_list as $buyer1){ ?>							
							<option value="<?php echo $buyer1['buyer_id'];?>" <?php if($buyer == $buyer1['buyer_id']){echo 'selected="selected"';}?> ><?php echo $buyer1['first_name']." ".$buyer1['last_name'] ;?></option>			
						<?php } } ?>
					   </select>
					</div>
				</div>
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<select class="form-control font-mark-form" id="sel1" name="seller">
							<option value="">Select</option>
							<?php if(count($seller_list) > 0){ 
							foreach($seller_list as $seller1){ ?>							
								<option value="<?php echo $seller1['seller_id'];?>" <?php if($seller == $seller1['seller_id']){echo 'selected="selected"';}?> ><?php echo $seller1['first_name']." ".$seller1['last_name'] ;?></option>			
							<?php } } ?>					 		
					   </select>
					</div>
				</div>
			...
			<div class="row">
				<div class="col-xs-12">							
					...
					<div class="table-header">
						Total orders between clients and freelancers
					</div>
						...
						<table id="examples" class="table table-striped table-bordered" cellspacing="0" width="100%">
							...
							<?php if(count($payment_list) > 0){
								...
								foreach($payment_list as $list){
									...
									$tot = $list['total_price'];
								?>
								<tr>
									<td><?php echo $i; ?></td>
									<td><?php echo $list['buyer_fname'] .' '.$list['buyer_lname'];?> </td>
